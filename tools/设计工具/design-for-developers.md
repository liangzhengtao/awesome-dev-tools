...
1. **快速草图** — Excalidraw 手绘风格，不追求完美，快速表达想法，支持实时协作
2. **架构图** — draw.io 丰富的形状库（AWS/Azure/GCP 图标），支持自动布局和多格式导出
3. **代码图表** — Mermaid 在 Markdown 中直接写图表代码，GitHub/GitLab/Notion 原生支持
4. **专业设计** — Figma Dev Mode 让开发者直接查看 CSS 属性和代码
5. **开源替代** — Penpot（Figma 替代）、tldraw（无限画布）支持自托管

### 模板说明

- Excalidraw 快捷键 — R（矩形）、O（椭圆）、A（箭头）、T（文字）、P（铅笔）
- Mermaid 语法 — 流程图、时序图、类图、ER 图、Git 图的完整示例
- draw.io 使用场景 — 架构图、流程图、ER 图、网络拓扑图
- 工具选择指南 — 什么场景用什么工具的决策树

### 常见陷阱

1. **过度追求完美** — 开发者的设计需求应注重快速表达，而非像素级完美
2. **不使用模板** — 大多数工具都提供模板库，从模板开始比从零开始高效得多
3. **导出格式不当** — 文档用 PNG，演示用 SVG（矢量放大不失真）
4. **Mermaid 语法错误** — 图表代码语法错误会导致渲染失败，使用 VS Code 预览插件实时检查
5. **忽略 Figma Dev Mode** — 开发者查看设计稿时应使用 Dev Mode 获取精确的样式参数
